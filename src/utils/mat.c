   ...
   const char * sep, const char * end, const char * suffix)
{
   ...
   const char * x_start = start ? start : "  ";   
   const char * x_dblfmt = dblfmt ? dblfmt : "%7.3f";
...
int cd_mat_mat_print(const char *prefix, const double *m, int row, int col)
{
  int i;
  printf("%s", prefix);
  for (i = 0; i < row; i++) {
    cd_mat_vec_print(NULL, m+col*i,col);
  }
